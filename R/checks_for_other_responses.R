...
extract_other_data_an <- function(input_tool_data, input_survey, input_choices) {
  ...
  for (cln in others_colnames) {
    ...
    df_filtered_data_an <- df_data_an %>% 
      ...
      mutate(other_name = cln,
             int.my_current_val_extract = if_else(str_detect(current_value, "other\\b"), str_extract_all(string = current_value, pattern = "other\\b|[a-z]+._other\\b"), current_value),
             value = "",
             parent_qn = current_parent_qn)
    df_other_response_data_an <- rbind(df_other_response_data_an, df_filtered_data_an)
  }  
  
  #arrange the data
  df_data_arranged_an <- df_other_response_data_an %>% 
    arrange(start_date, uuid)
  
  # get choices to add to the other responses extracted
  df_grouped_choices_an <- input_choices %>% 
    group_by(list_name) %>% 
  ...
  df_data_parent_qns_an <- df_data_arranged_an %>% 
    ...
    seperate(col = type, into = c("select_type", "list_name"), sep = "", remove = TRUE, extra = "drop") %>% 
    rename(name = parent_qn)
  
  # join other responses with choice options based on list_name
  df_join_other_responses_with_choices <- df_data_parent_qns_an %>% 
    left_join(df_grouped_choices_an, by = "list_name") %>% 
  ...
  output <- list()
  # select_one checks
  output$select_one <- df_join_other_responses_with_choices %>% 
    filter(str_detect(select_type, c("select_one|select one"))) %>% 
    mutate(type = "change_response")
  
  # select_multiple checks
  select_mu_data <- df_join_other_responses_with_choices %>% 
  ...
  merged_other_checks <- bind_rows(output) %>% 
    mutate(uuid_cl = paste(uuid, "_", type, "_", name),
           so_sm_choices = choice_options) %>% 
    select(uuid, start_date, enumerator_id, district_name, point_number, type, name, current_value, value, issue_id, issue, other_text, checked_by, checked_date, comment, reviewed, adjust_log, uuid_cl, so_sm_choices)
}
