...
>There are **`r nrow(df_updated_tool_data_an)`** total number of surveys done as of **`r df_days_for_data_collection_an[length(df_days_for_data_collection_an)]`**.

### Settlements:  **`r df_updated_tool_data_an %>% filter(status == "refugee") %>% nrow()`** surveys

```{r, echo = FALSE}
df_refugee_samp_per_settlement_an <- df_settlement_samples_required_an %>% 
...
```
### Host community: **`r df_updated_tool_data_an %>% filter(status == "host_community") %>% nrow()`** surveys

```{r, echo = FALSE}
df_host_samp_per_sub_county <- df_host_samples_required_an %>% 
...
The average survey time for all the data is: **`r round(mean(df_enum_performance_an$int.survey_time_interval), 0)`** Minutes

```{r, echo = FALSE}
df_enum_performance_an %>% 
